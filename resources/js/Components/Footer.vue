  ...
  <footer class="bg-gray-50" aria-labelledby="footer-heading">
    ...
    <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
      <div class="border-t border-gray-200 pt-8 lg:flex lg:items-center lg:justify-between xl:mt-0">
        <div>
          <h3 class="text-lg font-semibold text-gray-900 tracking-wider uppercase">
            Learn More About Our Products
          </h3>
          <p class="mt-2 text-base text-gray-500">
            Subscribe to receive our Product Profiles & Newsletters <i class="ml-4 fas fa-caret-right fa-lg"></i><i class="ml-2 fas fa-caret-right fa-lg"></i>
          </p>
        </div>
      ...
      <div class="mt-8 border-t border-gray-200 pt-8 md:flex md:items-center md:justify-between">
        <div class="flex space-x-6 md:order-2">
          <a v-for="item in navigation.social" :key="item.name" :href="item.href" class="text-gray-600 hover:text-blue-600">
            <span class="sr-only">{{ item.name }}</span>
            <component :is="item.icon" class="h-6 w-6" aria-hidden="true" />
          </a>
        </div>
        <p class="mt-8 text-base text-gray-600 md:mt-0 md:order-1">
          &copy; {{ new Date().getFullYear() }} Talk Duka. All rights reserved.
        </p>
      </div>
    </div>
  </footer>
